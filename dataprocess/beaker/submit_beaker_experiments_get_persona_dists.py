...
DESCRIPTION = (
    """submit author persona distance jobs to beaker (using the AUTHOR_IDS environment variable)"""
)
...
def create_experiment(beaker: Beaker, author_id_list: List, experiment_idx: int) -> str:
    ...
    experiment_name = f"get_author_persona_distances_min-year-2012_run01_{experiment_idx:04d}"
    command = [
        "python",
        "-u",
        "scripts/get_persona_distances.py",
    ...
    spec = ExperimentSpec(
        ...
        tasks=[
            TaskSpec(
                name="get_persona_distances",
                image=ImageSource(beaker=DEFAULT_BEAKER_IMAGE_ID),
                command=command,
                datasets=[
                    DataMount(
                        source=DataSource(beaker="01GT7QB5ZBCK51XX22P4GC9R9F"), # result of get avg embeddings min-year 2012
                        mount_path="/data/bridger_embeddings",
                    ),
                    DataMount(
                        source=DataSource(beaker="01GTT114YZBH2NN8Y2MGRP0MBX"), # result of get-avg-specter-min-year-2012
                        mount_path="/data/specter_embeddings",
                    ),
                    DataMount(
                        source=DataSource(beaker="01GGQFE7F8Q503YX004KRXQRGK"),
                        mount_path="/data/final-processing-embeddings",
                    ),
                    DataMount(
                        source=DataSource(beaker="01GGQHT6K90QDD00KHZXG454DY"),
                        mount_path="/data/paper_authors",
                    ),
                    DataMount(
                        source=DataSource(beaker="01GWDJS1W2TV042XARRAR2E0QE"),
                        mount_path="/data/personas",
                    ),
                ],
                ...
                resources=TaskResources(memory="150GiB"),
